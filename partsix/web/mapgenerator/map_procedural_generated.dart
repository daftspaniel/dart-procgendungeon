...
class PGMap {
  ...
  void addRandomItem(int count, int itemID){
    ...
    for (int rc = 0; rc < count; rc++) {
      ...
      while (attempts<100 && placed==false){
        int rx = RND(Width);
        int ry = RND(Height);
        Block b = getBlock(rx, ry);
        if (isBlockWalkable(b.base)){
          b.addItem(itemID);
          placed = true;
        }
        attempts++;
  ...
  void createDoor() {
    MapRectangle topmostRoom = Rooms.first;

    Rooms.forEach(
        (r) {
        if (r.getTopMidPoint().y<topmostRoom.y) topmostRoom = r;
        }
    );

    setBlock(topmostRoom.getTopMidPoint().x, topmostRoom.y-1, DOOR, true);
  ...
  void updatePlayer(p1, findAction) {
    Block b = getBlock(p1.x, p1.y);
    if (b.items!=null)
    {
      findAction();
      b.items.removeLast();
      if (b.items.length==0) b.items = null;
    }
  }


  List addNPCs(int count, int NPCID) {
    ...
    for (int rc = 0; rc < count; rc++) {
      ...
      Block b = getBlock(rx, ry);
      if (b.character == VOID && isBlockWalkable(b.base)) {
        b.character = NPCID;
        npcs.add([rx,ry]);
      }
    }
