...
List<Future> loadImages() {
  ...
  ninja = new ImageElement(src: "assets_img/ninja.png");
  return [wall.onLoad.first, floor.onLoad.first, grass.onLoad.first, corner.onLoad.first, tree.onLoad.first, player.onLoad.first, diamond.onLoad.first, shrub.onLoad.first, ninja.onLoad.first];
...
void drawMap(CanvasRenderingContext2D ctx,
             PGMap Amap, int tileSize, RPGCharacter p1, int viewport,
             List npcs) {
      ...
      if (b != null) {
        ...
        if (b.items != null) {
          b.items.forEach((item) {
            ...
            if (item == DIAMOND) ctx.drawImageScaled(diamond, DX, DY, tw, tw);
          });
        }

        if (x == p1.x && y == p1.y) ctx.drawImageScaled(player, DX, DY, tw, tw);

        //if (b.character == NINJA) ctx.drawImageScaled(ninja, DX, DY, tw, tw);

        npcs.forEach((n) {
          if (x == n.x && y == n.y)
            ctx.drawImageScaled(ninja, DX, DY, tw, tw);
        } );
